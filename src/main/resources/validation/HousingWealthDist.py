...
chunk = chunk[["NetPropertyWealth", "GrossPropertyWealth", "GrossHousingWealth", "Weight", "Tenure"]]
...
if plotResults:
    # Read model results
    results = readResults(rootResults + r"/HousingWealth-run1.csv", start_time, end_time)
    # Keep only selected population
    if population == "all":
        pass
    elif population == "non-owners":
        nProperties = readResults(rootResults + r"/NHousesOwned-run1.csv", start_time, end_time)
    ...
    if printPlots:
        plt.tight_layout()
        plt.savefig("/home/carro/Validation-HousingWealth-{}.pdf".format(population), dpi=300)
    else:
        plt.show()
